...
export default {
  ...
  methods: {
    ...
    editParams () {
      this.$refs.editFormRef.validate(async valid => {
        ...
        if (res.meta.status !== 200) { return console.log(this.$message.error('修改参数失败！')) }
    ...
    // 文本框失去焦点,或按下的Enter键都会触发
    // trim()方法的作用是去掉字符串头尾的空格.
    // https://www.cnblogs.com/hnrainll/archive/2011/11/17/2252666.html
    async handleInputConfirm (row) {
      ...
      // 如果没有return，内容需要后续处理
      // row.inputValue.trim()加.trim防止前后有空格
    ...
    async saveAttrVals (row) {
      const { data: res } = await this.$http.put(`categories/${this.cateId}/attributes/${row.attr_id}`, {
        ...
        attr_vals: row.attr_vals.join(' ')// join('空格')用空格做拼接
      })
    ...
    // 删除对应的参数可选项
    handleClose (i, row) {
      row.attr_vals.splice(i, 1)// splice() 方法向/从数组中添加/删除项目--i添加/删除项目的位置--1要删除的项目数量
      this.saveAttrVals(row)
    }
  },
  // 添加新节点  computed 计算属性
  ...
  computed: {
    ...
    isBtnDisabled () {
      ...
      return false
      // 这里可以优化 return this.selectedCateKeys.length !== 3
    },
    // 当前选中的三级分类的ID
    cateId () {
      if (this.selectedCateKeys.length === 3) {
